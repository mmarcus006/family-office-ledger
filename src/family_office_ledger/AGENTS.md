...
├── domain/         # entities, transactions, reconciliation, value objects
├── services/       # business logic (ingestion, reconciliation, reporting, lot matching)
...
| CLI commands | cli.py | init, status, ingest, reconcile (7 subcommands), ui |
| Domain models | domain/ | dataclasses with UUID ids, UTC timestamps |
| Services | services/ | *Impl suffix, interface-based DI |
| Repositories | repositories/ | interfaces.py + sqlite.py/postgres.py |
| API | api/ | FastAPI factory, 6 routers |
| UI | ui/ | NiceGUI pages/components/state |
